...
const StudentBilling = () => {
  ...
  const billDetails = useMemo(() => {
    ...
    familyMembers.forEach(student => {
      ...
      // TRANSPORT LOGIC FIX: "transportFees" field is used here
      const tFees = Number(student.transportFees || 0);
      if (tFees > 0 && studentSelected.length > 0) {
        studentItems.push({ 
            name: "Transport Fees", 
            rate: tFees, 
            count: studentSelected.length, 
            total: tFees * studentSelected.length, 
            feeKey: "Transport Fees",
            isChecked: !studentUnchecked.includes("Transport Fees")
        });
      }
          ...
          <div className="bg-white p-6 rounded-2xl border-2 border-dashed border-slate-300">
            ...
            <div className="flex gap-3 mb-4">
              ...
              <button type="button" onClick={() => { 
                const name = document.getElementById('ex_name').value;
                const amt = document.getElementById('ex_amt').value;
                addExtraCharge(name, amt); 
                document.getElementById('ex_name').value=''; 
                document.getElementById('ex_amt').value=''; 
              }} className="bg-indigo-600 text-white px-5 rounded-xl font-bold active:scale-95 shadow-md"><PlusCircle size={20}/></button>
            </div>
            {extraCharges.map(ex => (
