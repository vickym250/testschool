...
const fixDataForForm = (data) => {
  ...
  // Date conversion logic for display (dd-mm-yyyy to yyyy-mm-dd)
  const formatToInputDate = (dateStr) => {
    if (!dateStr || !dateStr.includes("-")) return dateStr;
    const parts = dateStr.split("-");
    if (parts[0].length === 2) { 
      // Agar 12-03-2026 hai toh use 2026-03-12 banao
      return `${parts[2]}-${parts[1]}-${parts[0]}`;
    }
    return dateStr;
  };

  cleanData.dob = formatToInputDate(data.dob);
  cleanData.admissionDate = formatToInputDate(data.admissionDate);

  // ... rest of your gender/category logic
  return cleanData;
};
